    ...
    def __getitem__(self, i):
        ...
        with h5py.File(self.path + "sentinel/" + '.'.join(name.split('.')[:-1]) + ".h5", 'r') as file:
            ...
            if 's2' in self.modalities:
                ...
                N_dates = len(output["s2_dates"])
                N_data = len(output["s2"])
                N = min(N_dates, N_data)  # Use the minimum length to avoid index errors
                if N > 50:
        ...
        if 's1-asc' in self.modalities:
            ...
            N_dates = len(output["s1-asc_dates"])
            N_data = len(output["s1-asc"])
            N = min(N_dates, N_data)  # Use the minimum length to avoid index errors
            if N > 50:
                random_indices = torch.randperm(N)[:50]
        ...
        if 's1-des' in self.modalities:
            ...
            N_dates = len(output["s1-des_dates"])
            N_data = len(output["s1-des"])
            N = min(N_dates, N_data)  # Use the minimum length to avoid index errors
            if N > 50:
                random_indices = torch.randperm(N)[:50]
        ...
        if 's1' in self.modalities:
            ...
            N_dates = len(output["s1_dates"])
            N_data = len(output["s1"])
            N = min(N_dates, N_data)  # Use the minimum length to avoid index errors
            if N > 50:
                random_indices = torch.randperm(N)[:50]
